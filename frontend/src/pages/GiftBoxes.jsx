...
const MOCK_GIFT_BOXES = mockProducts.filter(p => p.category === 'Gift Box');
const PAGE_SIZE = 12;

export default function GiftBoxes() {
    const [products, setProducts] = useState(MOCK_GIFT_BOXES);
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                params.append('categories', 'Gift Box');
                ...
                // Only replace mock with API when every product has an image (so pics don't disappear)
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '120px 24px 60px', minHeight: '100vh' }}>
            ...
            <header style={{ marginBottom: '40px', textAlign: 'center' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto 32px' }}>
                    Curated assortments of premium treats and treasures.
                </p>
            </header>
